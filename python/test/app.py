...
from logger_setting import info, error, debug, warning
...
class test_app:
    ...
    def insert_value(self, id = None, class_name = None, name = None, xpath = None, link = None, message = ""):
        try: 
            ...
            info(f"Inserindo valor: '{message}' no campo: '{value}'")
    ...
    def wait_text_to_be_present(self, id = None, class_name = None, name = None, xpath = None, link = None, text = ""):
        try:
            value = self.getWebElement(id, class_name, name, xpath, link)            

            return WebDriverWait(self.driver, 14).until(
                EC.text_to_be_present_in_element_value(value, text)
            )
        except InvalidSelectorException as e:
            debug(f"InvalidSelectorException. Elemento não encontrado: {e}")
            return None
        except TimeoutException as e:
            debug(f"TimeoutException. Elemento não encontrado: {e}")
            return None        
    ...
    # aplicar esse método visibility_of_element_located
    # aplicar esse método text_to_be_present_in_element_value
    ...
    def edge_driver(self):
        try:
            edge_optional = EdgeOptions()
            edge_optional.add_argument("--disable-notifications")

            driver = webdriver.Edge(options=edge_optional)
